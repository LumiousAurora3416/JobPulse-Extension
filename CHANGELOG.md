...
- **实测表内现有 4 组重复**（星尘智能 ×3、杭州新中大 ×2、科大讯飞 ×2、光大证券 ×2，多出 5 条）——查重上线后不会再新增；历史重复记录需手动清理
- **新增只读脚本 `agent/find_duplicates.py`**：列出上述重复记录，输出每组各条的区分信息（结果 / 投递天数 / 匹配分 / 薪资 / JD 长度 + record_id）并按「结果进展」排序，便于判断留哪条、删哪条。**纯只读，不含任何写操作**；判定规则与插件、Bot 一致（公司 AND 岗位同时相同）。可选 `FEISHU_WEB_DOMAIN` 环境变量打印可点开的记录链接（API 不返回记录 URL，租户域名无法自动探测，故做成可选）
  - ⚠️ 一个真实教训：星尘智能那组里有一条是 **1 天前投的、还带着匹配分 73**（另两条是 53/49 天前的简历挂）——这是**重新投递**而非误重复。说明「公司+岗位」规则无法区分"手滑写两次"和"隔了两个月重新投"，**清理历史重复时必须人工看，不能机械删**；而插件侧的两步确认正好覆盖这种情况（会提示但允许强制写入）
