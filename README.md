...
- **待投递状态**：标记"想投暂未投"的岗位，避免遗忘且不触发跟进提醒
- **写入前查重**：同「公司 + 岗位」已有记录时**先提示、不写入**，确认要重复写再点一次（同公司不同岗位不算重复）
- **企业性质**：下拉选择（默认「民营企业」，可改），用于按企业性质切分投递数据
- 飞书**数据看板**：投递漏斗图、公司分布环形图、投递时间线

### Agent 侧
- **每日跟进提醒**：状态还在流程中（简历/测评/面试）且投递 ≥5 天的，自动推送飞书卡片。**每天最多 10 张、最久没跟进的优先、同一条 7 天内不重复**——避免"每天几十张、一张都不看"
- **交互按钮**：卡片按钮**按当前进展动态生成**（简历 → 进面试/测评/简历挂；面试 → 一面挂/二面挂/三面挂/offer），点一下表格和卡片同步更新
- **状态机驱动**：要不要继续提醒由「结果」推导——到终态（各种挂 / offer / 无反馈 / 放弃）自动停止提醒
- **面试日程提醒**：面试时间未来 1-2 天的自动推送提醒
- **数据统计**：投递总量 / 待投递 / 进行中 / offer / 已挂 / 无结论结束（按状态机分桶）
...
| 投递链接 | URL | 当前页 URL |
| 投递时间 | 日期 | 插件写入 |
| 结果 | 单选 | **投递状态机**：待投递 / 简历 / 测评 / 面试 / 简历挂 / 一面挂 / 二面挂 / 三面挂 / offer / 无反馈 / 放弃 |
| 提醒状态 | 公式 | `IF(OR({结果}="简历",{结果}="测评",{结果}="面试"),"待跟进","已静音")`——纯展示，代码不读写 |
| 上次提醒日期 | 日期 | Agent 推送后写入，用于控制复催节奏 |
| 投递天数 | 公式 | `DAYS(TODAY(), {投递时间})` |
| 消息ID | 文本 | Agent 自动写入 |
| 面试时间 | 日期 | Agent 写入或手动填写 |
| 企业性质 | 单选 | 央国企 / 民营企业 / 外企 / 其他（插件默认选「民营企业」，可改） |
| 匹配分 | 数字 | 插件算匹配度后写入（可选功能） |
