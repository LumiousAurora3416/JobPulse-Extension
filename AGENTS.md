...
| 企业性质 | 单选 | 央国企 / 民营企业 / 外企 / 其他（v1.8.0）。插件下拉手选（**v1.9.1 起默认「民营企业」**，可改；选「不指定」则不写该列）、Bot 对话提到才写，**不做自动识别** |
...
- **扩展弹窗里别用 `window.confirm()` / `alert()`**：原生弹窗会抢焦点，有把 popup 直接关掉的风险，用户会误以为操作失败。需要"二次确认"就用**两步确认**（第一次点提示、再点一次执行）或自绘内联确认条。v1.9.1 的写入查重就是这么做的
- **写飞书前先查重**：同一「公司 + 岗位」重复写入会污染表格（实测攒了 4 组重复）。Bot 侧 `create_record` 有去重，插件侧 v1.9.1 才补上（`findDuplicateRecord`，规则与 Bot 一致）
