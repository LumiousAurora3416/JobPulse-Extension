...
```
高频追踪（每日）:
  飞书查表 → 筛选提醒状态=待跟进 且 投递天数≥3（排除结果=待投递的记录）
          → 发送跟进提醒卡片（含按钮回调）
...
- **单选字段写入不存在的选项会直接报错**（`FieldConvFail`），飞书**不会自动新增选项** → 必须先建列时就把选项带全（`property.options`）。「企业性质」的选项值同时存在于 3 处——`agent/init_match_tables.py` 的 `COMPANY_TYPE_OPTIONS`、`popup.html` 下拉框、`agent/message_agent.py` 的 `COMPANY_TYPES`，**改一处必须三处同步**，否则插件/Bot 写入静默失败或整条记录写不进去
